...
| Quadrature and ODE | `numax/integrate/quadrature.mojo` (`Tensor`), `numax/integrate/array/quadrature.mojo`, `numax/integrate/ode.mojo`, `numax/integrate/integrate.mojo` | Two tiers on the `numax.linalg` pattern. `numax.integrate` is `Tensor`: `trapezoid`, `simpson` and `cumulative_trapezoid` over sampled values with `scipy.integrate`'s own signatures, `Plain`-only and host-side -- MAX ships no quadrature of any kind, and a weighted sum of samples at `Dual` means nothing, so this is the Plain-only-surface outcome. `numax.integrate.array` integrates a `FloatLike` *function* at a fixed node count and is tier 1. The adaptive drivers are tier 2, including `solve_ivp_stiff`, whose Newton iteration takes `df/dy` from `Dual` |
