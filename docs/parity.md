...
| Elementwise math | `numax/core/elementwise.mojo` | `Plain`-only over `std.math`, rather than growing `FloatLike` by twenty methods across seven conformers; `exp`, `log` and `erf` come from `numax/core/libm.mojo` at float64, where `std.math`'s are `1e5` to `2e8` ulp off |
