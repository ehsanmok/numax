...
| Not yet | `svd`, `eigh`, `cond`, `pinv` over `Tensor`. `cond` is `svd`'s dependent and moves when it does. MAX's `qr_factorization` is on the older `LayoutTensor`, which numax denies rather than bridges, so `qr_factor` is numax's own |
