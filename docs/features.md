...
| Surface | Where |
|---|---|
| `save`, `load` — numax's own `NMX1` binary format: little-endian, dtype/rank/shape in the header and checked on load, because MAX ships no array I/O to interchange with | [`io/io.mojo`](../numax/io/io.mojo) |
