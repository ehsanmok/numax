...
| `Interval[Inner]` | An enclosure of every `f(x)` for `x` in `[lo, hi]`, including tight `sin`/`cos` over an interval. Exact for monotone kernels (`erf`, `erfc`, `exp`), valid but loose through polynomials (`j0`), and not an enclosure at all through an alternating rational approximation: `gamma` over `[1.5, 2.5]` is `[nan, nan]`, because Lanczos' sum crosses zero as an interval | [`core/interval.mojo`](../numax/core/interval.mojo), [`enclosures.mojo`](../examples/intermediate/enclosures.mojo) |
...
| Arithmetic and operators | `add`, `subtract`, `multiply`, `divide`, `power`, `mod`, `floor_divide`, `negative`, `invert`, `astype` — tensor-tensor, tensor-scalar and tensor-tensor at two broadcastable shapes; `astype` is explicit because there is no dtype promotion. Each takes `gpu: Bool = False` last, like the elementwise row below, and the operators `+`, `-`, `*`, `/`, unary `-` forward at that default, so `a + b` on a device tensor falls back to a host walk and names `add[gpu=True]` on `stderr` | [`core/ops.mojo`](../numax/core/ops.mojo) |
| Elementwise math | `exp`, `exp2`, `expm1`, `log`, `log2`, `log10`, `log1p`, `sqrt`, `rsqrt`, `cbrt`, `abs`, `sin`, `cos`, `tan`, `sinh`, `cosh`, `tanh`, `floor`, `ceil`, `trunc`, `round`, `copysign`, `arcsin`, `arccos`, `arctan`, `arctan2`, `arcsinh`, `arccosh`, `arctanh`, `hypot`, `maximum`, `minimum`, `clip`, `remainder`, `diff`, `gradient` — each takes `gpu: Bool = False` last, so `exp(a)` runs on the host and `exp[gpu=True](a)` on the device; asking for the target the tensor is not on falls back to a host walk and says so on `stderr` | [`core/elementwise.mojo`](../numax/core/elementwise.mojo) |
| Comparison and logic | `equal`, `not_equal`, `less`, `less_equal`, `greater`, `greater_equal`, `isclose`, `allclose`, `array_equal`, `isnan`, `isinf`, `isfinite`, `isposinf`, `isneginf`, `logical_and`, `logical_or`, `logical_not`, `logical_xor`, `all`, `any` — truth is a `Static[DType.bool]`, and `select`/`extract` take exactly that, so comparisons compose straight into a mask. Each comparison and predicate takes `gpu: Bool = False` last, like the two rows above, so `greater(a, b)` runs on the host and `greater[gpu=True](a, b)` writes its bool mask on the device; `all` and `any` return a `Bool` and stay host reads | [`core/logic.mojo`](../numax/core/logic.mojo) |
...
|---|---|
| Products | `matmul` (comptime and run-time shapes), `matvec`, `batched_matmul` — MAX's `linalg.matmul`/`bmm` outright, so they inherit its Apple/NVIDIA/AMD/vendor-BLAS dispatch and numax names no architecture. `inner` is that same kernel under its `transpose_b` parameter, so a Gram matrix costs no materialized transpose; `matrix_power` is a squaring chain of it; `kron` and `cross` (3-vectors and `n x 3` rows) are `elementwise` maps, MAX shipping no equivalent; `tensordot(a, b, axes)` in NumPy's integer form is one run-time-shape `matmul` on the two buffers retyped as matrices, returning a `Dynamic` at the combined rank |
...
| Structured constructors | `toeplitz` (two-argument and symmetric), `hankel`, `circulant`, `companion`, `hilbert`, `block_diag`, `khatri_rao`, `convolution_matrix`, `pascal`, `invpascal` (closed form), `hadamard`, `helmert`, `fiedler`, `fiedler_companion`, `leslie` — `scipy.linalg`'s `_special_matrices`, each one an `elementwise` map from an index rule. `companion`'s eigenvalues are its polynomial's roots, which is the bridge to `numax.linalg.array.eigvals`; `convolution_matrix` is the matrix `C` with `C @ v == convolve(a, v)`, which is what deconvolution needs and a function cannot be |
...
| Reductions | `sytrd` — a symmetric matrix to tridiagonal form, device-resident and blocked, returning a reusable `TensorTridiagonal` with `.q()`. Over half an `eigh`'s arithmetic and the half with a GEMM in it: a `latrd` panel of `block` columns defers the trailing work into one rank-`2 block` `[V \| W] @ [W \| V]^T` product under `transpose_b=True` per panel, where the unblocked form issued one per column, and `.q()` accumulates the packed reflectors by panels through `qr_factor`'s block-reflector walk. `block = 1` is the unblocked algorithm and `block = n` a single panel, both pinned by tests |
| General spectrum | `hessenberg` — a general matrix to upper Hessenberg form, device-resident and blocked over `lahr2` panels (the panel's `V`, `T` and `Y = A V T` accumulated per column and the two-sided update deferred into one `transpose_b=True` GEMM on the right and `larfb`'s three on the left, which took it from 1,461 ms to 146 at `n = 1024`), as a `TensorHessenberg` with `.h` and `.q()` (LAPACK's `gehrd`/`orghr`); `eigvals` — the eigenvalues of a general matrix as an `Eigenvalues` `(re, im)` pair, Francis double-shift QR on the Hessenberg form (EISPACK's `hqr`), in LAPACK's order with complex pairs adjacent; `schur` — the real Schur form `a = Z T Z^T` as a `TensorSchur`, `T` quasi-triangular with a complex pair in each `2 x 2` block and `Z` orthogonal, the form every matrix function past `expm` is built on |
| Symmetric eigen | `eigvalsh` and `eigh`, returning a `TensorEigh` of ascending values and orthonormal column vectors. `sytrd` reduces device-resident and blocked over `latrd` panels, implicit QL sweeps the two diagonals on the host (tier 2, declared), and `eigh`'s vectors come back through one `matmul` against the reduction's `Q`. The rotations are no longer scalar host work: they are grouped by Lang's windowing rule into batches of commuting pairs and each batch applied to `Z^T` as one GEMM, which took `eigh` at `n = 1024` from 6,101 ms to 199 (0.45 of LAPACK, M3 Pro, `float32`). The named ceiling is now the reduction's own whole-matrix `p = A v`, one per column, which only a two-stage dense-to-banded reduction removes; a values-only caller should pass `block=8` (74.8 ms against 142 at the default 32) |
| SVD | `svdvals` and `svd`, returning a `TensorSVD` of `u`, descending `s` and `v` (columns; SciPy's `Vh` is `transpose(v)`), for any `m >= n`. `gebrd` reduces to bidiagonal form device-resident and blocked over `labrd` panels, and `_bdsqr` -- LAPACK's implicit-shift bidiagonal QR, zero-shift where a shift would cost relative accuracy -- diagonalizes the band on the host while its two rotation streams accumulate `U` and `V` device-resident through the same windowed batches `eigh` uses. The `O(n^2)` band chase is the named ceiling |
| SVD dependents | `pinv` (`V diag(1/s) U^T`, one `inner` product after the SVD, NumPy's `rcond`), `cond` (the 2-norm ratio, `inf` when singular), `matrix_rank` (NumPy's default tolerance, or an explicit one), and `lstsq[method="svd"]`, which returns the minimum-norm solution a rank-deficient system has where the `"qr"` default cannot; `tensorsolve` and `tensorinv` read a tensor whose element count is a perfect square as the `M x M` matrix it is (fixed at compile time from the shape) and hand it to `solve` and `inverse`, the result retyped at the tensor's own trailing extents |
| Spectral on a device | **The six spectral entry points take `gpu: Bool = False` like the rest of the tier, and `gpu=True` is broken on Metal in 0.2**: `sytrd[gpu=True]` disagrees with the host band at `n = 4` at every `block` including 1, and `svd[gpu=True]` raises rather than converging. The factorizations above are unaffected. Use the default until it is fixed; `docs/performance.md`'s "What is not measured" carries the diagnosis |
| Matrix functions on the Schur form | `logm` (inverse scaling and squaring, Padé on the Schur factor), `funm` (block Parlett recurrence with any `FloatLike` scalar function, complex 2x2 blocks through `Complex[Plain]`), `cosm`/`sinm`, `fractional_matrix_power`, and the general `sqrtm` beside `expm` -- everything the `Array`-tier `matfuncs` waited on `schur` for |
...
| Reductions along one axis | `sum[axis=k](a)`, and the same second overload on `prod`, `min`, `max` and `mean`, plus `variance_axis[axis=k](a)` returning mean and variance from one traversal — `numpy.sum(a, axis=k)` and friends, at any rank above 1. The axis is a compile-time parameter and drops from the result, whose remaining extents are run-time values. These fold through `numax.core.rowwise`'s `sum_axis`/`prod_axis`/`min_axis`/`max_axis` and carry the same `gpu: Bool`; the axis-wise `argmin`/`argmax` route to `nn.argmaxmin` instead, MAX's only axis-taking entry point | [`stats/statistics.mojo`](../numax/stats/statistics.mojo) |
| Order statistics | `quantile`, `percentile` — under all thirteen `numpy.quantile` methods, one value or a tensor of probabilities; `nanquantile`, `nanpercentile`, `nanmedian`; `scipy.stats.iqr` with `interpolation` and `nan_policy`. Host-side by **selection, not a sort**: all thirteen methods read one or two order statistics, so a three-way quickselect places exactly those in `O(n)`; the vector-`q` overloads sort once instead when `3 m >= log2 n` | [`stats/quantiles.mojo`](../numax/stats/quantiles.mojo) |
...
| Correlation | `cov`, `corrcoef` for a pair or a variables-by-observations matrix — centering through MAX's `Welford` monoid, then `C C^T` as one `linalg.matmul` with `transpose_b=True`, so both take `gpu: Bool = False` and run where the tensor lives (the pair form stacks into a `2 x n` and reuses the matrix path); `pearsonr`, `spearmanr`, `kendalltau` (tau-b, asymptotic p) with two-sided p-values through `t.sf`/`norm.sf`; `linregress` with both standard errors; `rankdata` under all five tie methods; `zscore` | [`stats/correlation.mojo`](../numax/stats/correlation.mojo) |
...
| NaN-ignoring reductions | `nansum`, `nanprod`, `nanmean`, `nanvar`, `nanstd`, `nanmin`, `nanmax` — `isnan`, a `select` against a fill, and the plain reduction; the reduction runs on either target now but `select` is still host-side, so the composition is, and none of these forwards a `gpu` parameter yet. An all-NaN input raises where NumPy warns | [`stats/nanfunctions.mojo`](../numax/stats/nanfunctions.mojo) |
