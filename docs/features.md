...
| Not yet | `qr`, `svd`, `eigh`, `cond`, `pinv` over `Tensor`. MAX's `qr_factorization` is on the older `LayoutTensor`, which numax denies rather than bridges |
