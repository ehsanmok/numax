...
| **Tier 2** | Free to loop or branch on data; `Plain`-only, host-side unless the row says otherwise | `ops`, `logic`, `sorting`, `io`, `stats`'s tensor reductions, the `Tensor` tiers of `linalg`, `interpolate`, `fft` and `signal` (host-orchestrated; `gpu=True` still runs MAX's kernels), `optimize`'s converge-to-tolerance minimizers, `integrate`'s adaptive `quad`/`solve_ivp`. `ops`, `elementwise`, `logic` and `rowwise` are tier 2 in shape only: `Plain`-only, but one body on both targets |
