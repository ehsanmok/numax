...
**The BLAS-1 table lives in `bench_blas1_gpu.mojo`**, not here, and the
reason is a build limit rather than a methodological one: the two halves
together instantiate more GPU kernels than Apple's Metal compiler will put
in one metallib, so the combined file failed with "Metal Compiler failed
to compile metallib" before running anything. Either half builds alone --
checked by bisection at every size in the sweep -- so splitting them is
what makes the device numbers reachable on Metal at all. CUDA built either
way, and neither table's numbers change.
...
`max_batch_size=1` goes with it at every call site below, so `run` times
one launch-through-completion round trip per sample rather than amortizing
a batch of them across one synchronize. That is the same shape the
hand-rolled loop had and the same one `../torch/linalg.py` uses; a
factorization is a chain of dependent launches and cannot pipeline anyway.
"""
