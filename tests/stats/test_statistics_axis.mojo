...
def test_folding_every_axis_matches_the_whole_tensor_sum() raises:
    ...
    for axis in range(3):
        ...
        if axis == 0:
            var r = sum_axis[axis=0](a)
            for i in range(r.size()):
                partial += r[i]
        elif axis == 1:
            var r = sum_axis[axis=1](a)
            for i in range(r.size()):
                partial += r[i]
        else:
            var r = sum_axis[axis=2](a)
            for i in range(r.size()):
                partial += r[i]
...
def test_mean_axis_is_the_sum_divided_by_the_axis_length() raises:
    var a = _ramp[3, 4]()
    var totals = sum_axis[axis=0](a)
    var means = mean_axis[axis=0](a)
...
def test_an_axis_reduction_accepts_a_run_time_shape() raises:
    ...
    var from_static = sum_axis[axis=1](static_a)
    var from_dynamic = sum_axis[axis=1](dynamic_a)
