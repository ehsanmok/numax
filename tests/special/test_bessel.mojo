"""Tests for `numax.special.bessel` against known values and `std.math` references."""
...
from numax import Dual, Plain, j0, j1, y0, y1
...
def test_bessel_j0_at_zero_is_one() raises:
    ...
    assert_almost_equal(j0(x).v, SIMD[dtype, width](1))


def test_bessel_j0_matches_known_values() raises:
    # Reference values from standard Bessel function tables.
    var x1 = Plain[dtype, width].constant(1)
    assert_almost_equal(
        j0(x1).v, SIMD[dtype, width](0.7651976865579666), atol=1e-7
    )

    var x2 = Plain[dtype, width].constant(2)
    assert_almost_equal(
        j0(x2).v, SIMD[dtype, width](0.22389077914123567), atol=1e-7
    )

    # J0 crosses zero near x=2.405, so J0(3) is negative.
    var x3 = Plain[dtype, width].constant(3)
    assert_almost_equal(
        j0(x3).v, SIMD[dtype, width](-0.26005195490193343), atol=1e-7
    )


def test_bessel_j0_is_even() raises:
    var x = Plain[dtype, width].constant(2.5)
    var neg_x = Plain[dtype, width].constant(-2.5)
    assert_almost_equal(j0(x).v, j0(neg_x).v)
...
def test_bessel_j0_far_branch_matches_std_math() raises:
    ...
    for x64 in [3.5, 5.0, 8.0, 10.0, 20.0, 50.0, 100.0]:
        assert_almost_equal(
            j0(pv(x64)).v, SIMD[dtype, width](j0_f64(x64)), atol=1e-7
        )
...
def test_bessel_y0_matches_std_math_near_and_far() raises:
    for x64 in [0.1, 0.5, 1.0, 2.0, 2.9, 3.0, 4.0, 8.0, 20.0, 50.0]:
        assert_almost_equal(
            y0(pv(x64)).v, SIMD[dtype, width](y0_f64(x64)), atol=1e-6
        )
...
def test_bessel_y1_matches_std_math_near_and_far() raises:
    for x64 in [0.1, 0.5, 1.0, 2.0, 2.9, 3.0, 4.0, 8.0, 20.0, 50.0]:
        assert_almost_equal(
            y1(pv(x64)).v, SIMD[dtype, width](y1_f64(x64)), atol=1e-6
        )
...
def test_bessel_j0_derivative_is_negative_j1() raises:
    ...
    for x64 in [1.0, 2.0, 5.0, 10.0]:
        var x = D(pv(x64), pv(1))
        var j0d = j0(x)
        assert_almost_equal(j0d.deriv.v, -j1(pv(x64)).v, atol=1e-6)
