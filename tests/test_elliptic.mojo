"""Tests for `numax.elliptic` against known closed-form values, a
...
def agm_reference(m: Float64) -> Tuple[Float64, Float64]:
    ...
    `numax.elliptic`'s own A&S polynomial approximation.
    ...
    17.3.36's misdigitized `b4` coefficient -- see `numax/elliptic.mojo`.
    """
    var a = Float64(1.0)
...
def test_elliptic_e_at_one_is_one() raises:
    ...
    # formula right there -- see `numax/elliptic.mojo`'s module docstring.
    assert_almost_equal(
...
def test_elliptic_k_diverges_but_stays_finite_at_one() raises:
    # K(1) is a true mathematical singularity; `numax.elliptic` caps it at
