...
`erf`/`erfc` live in `ember.erf`; `gamma`/`lgamma`/`gammainc`/`gammaincc` in
`ember.gamma`; `bessel_j0` in `ember.bessel`; `lambertw` in `ember.lambertw`
-- this module keeps the activations (`gaussian`, `sigmoid`, `swish`,
`tanh`, `relu`, `leaky_relu`, `gelu`) plus `softmax`, matching `ember`'s
...
function over `Plain` `SIMD` values, built from `ember.tensor`'s `reduce_rows`
...
from .tensor import add_op, broadcast_op_rows, max_op, reduce_rows
...
def _wrap_plain[dtype: DType](x: SIMD[dtype, 1]) -> Plain[dtype, 1]:
    return Plain[dtype, 1](x)


def _unwrap_plain[dtype: DType](p: Plain[dtype, 1]) -> SIMD[dtype, 1]:
    return p.v


def _sub_exp_op[
    dtype: DType
](a: Plain[dtype, 1], b: Plain[dtype, 1]) -> Plain[
    dtype, 1
] where dtype.is_floating_point():
    # exp(a - b), fused into one pass so `softmax` doesn't need a separate,
    # in-place elementwise `exp` step (which `map`/`map_simd` can't express
    # anyway -- their `xs`/`ys` parameters may not alias the same buffer).
    return (a + (-b)).exp()


def _div_op[
    dtype: DType
](a: Plain[dtype, 1], b: Plain[dtype, 1]) -> Plain[dtype, 1]:
    return a / b
...
def softmax[
    ...
    are caller-provided scratch space -- `ember.tensor`'s primitives never
    ...
    Built entirely from `ember.tensor.reduce_rows` and
    `ember.tensor.broadcast_op_rows`: a per-row max, a fused
    subtract-and-`exp` broadcast into `tmp`, a per-row sum of `tmp`, then a
    divide broadcast into `ys`.
    """
    reduce_rows[
        wrap=_wrap_plain[dtype],
        unwrap=_unwrap_plain[dtype],
