...
trait FloatLike(Copyable, Deinitable, Movable):
    ...
    def sqrt(self) -> Self:
        ...
        reason enough" rule -- `numax.bessel`'s `_inv_sqrt` and
        `numax.lambertw`'s branch-point seed each spelled it
...
def guard_nonzero[T: FloatLike](x: T, floor: T) -> T:
    ...
    it" -- Lentz's continued-fraction guard in `numax.beta`, and the
    zero-derivative guard in `numax.solve`. `copysign`'s `+1`-at-zero
...
def default_erf_approx[T: FloatLike](x: T) -> T:
    ...
    rather than in `numax.erf`, so those base numeric types don't have to
    ...
    error near `x = 0`; see `tests/test_compensated.mojo`), and `Dual.erf()`
