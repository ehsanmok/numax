...
`erf`/`erfc` live in `numax.erf`; `gamma`/`lgamma`/`gammainc`/`gammaincc` in
`numax.gamma`; `bessel_j0` in `numax.bessel`; `lambertw` in `numax.lambertw`
...
function over `Plain` `SIMD` values, built from `numax.tensor`'s `reduce_rows`
...
from .tensor import add_combine, broadcast_op_rows, max_combine, reduce_rows
...
def softmax[
    ...
    are caller-provided scratch space -- `numax.tensor`'s primitives never
    ...
    Built entirely from `numax.tensor.reduce_rows` and
    `numax.tensor.broadcast_op_rows`: a per-row max, a fused
