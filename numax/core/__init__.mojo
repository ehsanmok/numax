...
| `tensorlike` | `TensorLike`, the bound every routine below takes its tensors through, with `Tensor` (owned) and `View` (a borrowed `TileTensor` plus its device) as the conformers; `dim[T, i]` and `is_row_major[T]` for signatures |
