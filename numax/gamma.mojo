...
def gammainc[T: FloatLike](a: T, x: T) -> T:
    ...
    Gamma(a+1)` at negative non-integer `a` (see `tests/test_gamma.mojo`).
    ...
    # (and everything built on it) CPU-only. See `numax.orthopoly`'s module
    # docstring.
    var nf = T.one()
...
def digamma[T: FloatLike](x: T) -> T:
    ...
    its own reflection formula. `tests/test_gamma.mojo` was already
