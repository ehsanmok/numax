...
`erf`/`erfc` are `FloatLike` trait methods (see `numax.numeric`), not
...
`tests/test_compensated.mojo`), `Dual` differentiates through the chain
rule over its `Inner`'s own `erf()`, and `Compensated`/`Decimal` fall back
to `numax.numeric.default_erf_approx`, the Abramowitz & Stegun 7.1.26
