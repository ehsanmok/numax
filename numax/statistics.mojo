...
- **`Plain`-only, `TileTensor`-based** (`sum`, `prod`, `min`, `max`, `mean`,
  ...
  compose from `numax.tensor.reduce` the same way `numax.array`'s creation
...
- **`FloatLike`-generic, `List[T]`-based** (`mean`, `variance`, `stddev`,
  ...
  precision motivation `numax.compensated`'s own tests already measure
  (`tests/test_compensated.mojo`), applied to a running sum instead of a
...
from .tensor import add_combine, max_combine, reduce
...
def variance[T: FloatLike](xs: List[T], ddof: Int = 0) -> T:
    ...
    the same way any long summation does (`numax.compensated`'s own tests
