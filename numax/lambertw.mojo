...
Both use Halley's method (see `numax.gamma`'s docstring for why a fixed
...
- `lambertw_m1` has no such luck with a single seed shape: the branch-point
  ...
  picks between them with the same branchless blend `numax.gamma`'s
  reflection and `numax.bessel`'s near/far split use: both seeds are always
...
def lambertw[T: FloatLike](x: T) -> T:
    ...
    of times rather than to a data-dependent tolerance (see `numax.gamma`'s
