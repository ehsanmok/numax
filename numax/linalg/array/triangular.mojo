"""Triangular and tridiagonal solves over `Array`.
...
inside a GPU thread at any conformer. These are the primitives the
`Array` factorizations in `array.cholesky`, `array.lu` and `array.qr`
finish with.
...
tractable. It is the one name here that will never gain a `Tensor` form:
Thomas is already linear and has nothing to hand a GEMM. The blocked
`Tensor` triangular solve is `numax.linalg.triangular`'s
`solve_triangular`.
"""
