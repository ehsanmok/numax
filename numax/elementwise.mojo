...
**This module is tier 2**, on the same terms as `numax.logic`: each routine
...
`numax.tensor.map`/`map_to` are the GPU-launchable primitives that express
...
`Plain`-only. `FloatLike` deliberately stays small (`numax/numeric.mojo`
...
`variance`, `stddev` and `numax.logic.all_true`. `nextafter` is absent from
