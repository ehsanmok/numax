...
this module (see `tests/test_elliptic.mojo`, which ports it).
...
floored at a small epsilon (branchless, via `numax.tensor.max_op`'s
`max(a,b) = (a+b+|a-b|)/2` identity, same as `numax.bessel`'s domain
